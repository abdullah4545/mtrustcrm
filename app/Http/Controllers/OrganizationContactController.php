...
class OrganizationContactController extends Controller
{
    ...
    public function datatable(Organization $organization)
    {
        ...
        return DataTables::of($q)
            ->addIndexColumn()
            ->editColumn('phone', fn($row) => e(implode(', ', $row->phone_numbers ?: array_values(array_filter([$row->phone,$row->phone_two])))))
            ->editColumn('email', fn($row) => e(implode(', ', $row->email_addresses ?: array_values(array_filter([$row->email])))))
    ...
    public function store(Request $request, Organization $organization)
    {
        ...
        $phones = collect($request->input('phone_numbers', [$request->phone,$request->phone_two]))->filter()->values();
        $emails = collect($request->input('email_addresses', [$request->email]))->filter()->values();
        $contact->phone_numbers = $phones->all();
        $contact->email_addresses = $emails->all();
        $contact->phone = $phones->get(0);
        $contact->phone_two = $phones->get(1);
        $contact->email = $emails->get(0);
