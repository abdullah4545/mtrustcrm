...
@section('maincontent')
<div class="nxl-content"><div class="page-header"><div class="page-header-title"><h5 class="m-b-10">Lead Follow-ups</h5></div></div></div>
<div class="main-content">@if(session('message'))<div class="alert alert-success">{{ session('message') }}</div>@endif
<div class="row g-3 mb-3"><div class="col-md-4"><a class="text-decoration-none" href="{{ route('followups.index',['filter'=>'overdue']) }}"><div class="card p-3 border-danger"><small>Overdue</small><h3 class="text-danger mb-0">{{ $counts['overdue'] }}</h3></div></a></div><div class="col-md-4"><a class="text-decoration-none" href="{{ route('followups.index',['filter'=>'today']) }}"><div class="card p-3 border-primary"><small>Today</small><h3 class="text-primary mb-0">{{ $counts['today'] }}</h3></div></a></div><div class="col-md-4"><a class="text-decoration-none" href="{{ route('followups.index',['filter'=>'upcoming']) }}"><div class="card p-3 border-success"><small>Next 7 Days</small><h3 class="text-success mb-0">{{ $counts['upcoming'] }}</h3></div></a></div></div>
<div class="card"><div class="card-body"><form class="row g-2 mb-3" method="GET"><div class="col-md-3"><select name="filter" class="form-control"><option value="today" @selected($filter==='today')>Today</option><option value="overdue" @selected($filter==='overdue')>Overdue</option><option value="upcoming" @selected($filter==='upcoming')>Next 7 Days</option><option value="all" @selected($filter==='all')>All</option></select></div>@can('staff.filter')<div class="col-md-3"><select name="user_id" class="form-control"><option value="">All Staff</option>@foreach($staffs as $staff)<option value="{{ $staff->id }}" @selected((string)request('user_id')===(string)$staff->id)>{{ $staff->name }}</option>@endforeach</select></div>@endcan<div class="col-md-4"><input name="q" value="{{ request('q') }}" class="form-control" placeholder="Search contact, phone, lead no"></div><div class="col-md-2 d-flex gap-1"><button class="btn btn-primary flex-fill">Search</button><a href="{{ route('followups.index') }}" class="btn btn-light">Reset</a></div></form>
