...
@section('title')
    {{ ($business?->business_name ?? 'Medi Trust Solution') }} - Edit Sale
@endsection
...
<div class="nxl-content">
    ...
    <div class="page-header">
        ...
        <div class="page-header-right ms-auto d-flex gap-2">
            <a href="{{ route('sales.show',$sale->id) }}" class="btn btn-secondary"><i class="feather-eye me-1"></i> View</a>
                ...
                <form method="POST" action="{{ route('sales.update',$sale->id) }}">
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <input class="form-control" value="{{ $sale->invoice_no }}" readonly>
                        </div>

                        <div class="col-md-4">
                            <label class="form-label">Client Name</label>
                            <input class="form-control" name="client_name" value="{{ $sale->client_name }}">
                        </div>

                        <div class="col-md-4">
                            <label class="form-label">Client Phone</label>
                            <input class="form-control" name="client_phone" value="{{ $sale->client_phone }}">
                        </div>

                        <div class="col-md-4">
                            <label class="form-label">Client Email</label>
                            <input class="form-control" name="client_email" value="{{ $sale->client_email }}">
                        </div>

                        <div class="col-md-4">
                            <label class="form-label">Sale Date</label>
                            <input type="date" class="form-control" name="sale_date" value="{{ optional($sale->sale_date)->format('Y-m-d') }}">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-control" name="status_stage_id">
                                ...
                                @foreach($statuses as $s)
                                    <option value="{{ $s->id }}" {{ $sale->status_stage_id==$s->id?'selected':'' }}>{{ $s->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" step="0.01" class="form-control" name="discount_amount" id="discount_amount" value="{{ $sale->discount_amount }}">
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="form-check mt-2">
                                ...
                                <input class="form-check-input" type="checkbox" name="tax_enabled" id="tax_enabled" value="1" {{ $sale->tax_enabled ? 'checked' : '' }}>
...
const PRODUCT_DETAILS_URL = "{{ url('products') }}";
let rowIndex = 0;
const EXISTING_ITEMS = @json($sale->items);
...
function addRow(item = null){
    ...
    tr.innerHTML = `
        ...
        <td><textarea class="form-control item_desc" name="items[${idx}][description]" rows="2"></textarea></td>
        ...
        <td class="text-center"><button type="button" class="btn btn-sm btn-danger btn-remove">&times;</button></td>
    `;
    ...
    tr.querySelector('.btn-remove').addEventListener('click', () => { tr.remove(); calcTotals(); });
    ...
    tr.querySelector('.product_id').addEventListener('change', async (e) => {
        ...
        if(json.status){
            tr.querySelector('.item_name').value = json.data.name || '';
            tr.querySelector('.unit_price').value = json.data.sale_price || 0;
            tr.querySelector('.tax_rate').value = json.data.tax_rate || 0;
            calcTotals();
        }
    });
}
