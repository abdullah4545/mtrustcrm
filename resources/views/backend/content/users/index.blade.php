...
<div class="d-flex justify-content-between align-items-center gap-2 mb-2"><div><h6 class="mb-0">District / Upazila Data Access</h6></div><button type="button" class="btn btn-sm btn-outline-primary touch-btn" id="addArea"><i class="feather-plus"></i> Add District</button></div>
<div id="areaRows" class="d-grid gap-2"></div>
<div class="alert alert-info py-2 mt-3 mb-0"><b>Example:</b> Dhaka = All Upazilas; Gazipur = Sreepur + Kaliakair. If an area is assigned, this user will only access organizations inside those District/Upazila areas. Leave empty for no geographic restriction (permissions still apply).</div>
</div></div>
...
$(function(){
 ...
 $(document).on('change','.area-all',function(){const row=$(this).closest('.area-row');row.find('.area-upazilas').prop('disabled',this.checked);if(this.checked)row.find('.area-upazilas').val([]);row.find('.area-upazilas').trigger('change.select2')});
...
function clearForm(){document.getElementById('userForm').reset();$('#user_id').val('');$('#areaRows,#imgPreview').html('');$('#status').val('1');$('#workAreaSection').show();addAreaRow()}
